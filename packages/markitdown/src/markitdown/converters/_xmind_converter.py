...
class XMindConverter(DocumentConverter):
        ...
        **kwargs: Any,
        ...
        try:
            with zipfile.ZipFile(file_stream) as archive:
                ...
                if name is None:
                    raise ValueError("XMind archive is missing content.json or content.xml")
        ...
        sheets = self._parse_json(content) if name == "content.json" else self._parse_xml(content)
        if not sheets:
    ...
    def _parse_xml(content: bytes) -> list[tuple[str, dict[str, Any]]]:
        ...
        for sheet in XMindConverter._children(root, "sheet"):
            ...
            if topic is not None:
                ...
                sheets.append(
                    (XMindConverter._element_text(sheet_title), XMindConverter._xml_topic(topic))
                )
        return sheets

    @staticmethod
    def _xml_topic(topic: ElementTree.Element) -> dict[str, Any]:
    ...
    @staticmethod
    def _children(element: ElementTree.Element, local_name: str) -> Iterator[ElementTree.Element]:
    ...
    def _render_topic(cls, topic: dict[str, Any], depth: int) -> list[str]:
        ...
        note = cls._text(note)
        if note and title:
            lines.extend(f"{indent}  > {line}" if line else f"{indent}  >" for line in note.splitlines())
