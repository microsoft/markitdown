...
def _convert(stream: io.BytesIO) -> str:
    return (
        MarkItDown()
        .convert_stream(stream, stream_info=StreamInfo(extension=".xmind"))
        .markdown
    )
...
def _mark_entry_encrypted(stream: io.BytesIO) -> io.BytesIO:
    ...
    for signature, flag_offset in ((b"PK\x03\x04", 6), (b"PK\x01\x02", 8)):
        ...
        flags = int.from_bytes(
            archive[offset + flag_offset : offset + flag_offset + 2], "little"
        )
        archive[offset + flag_offset : offset + flag_offset + 2] = (flags | 1).to_bytes(
            2, "little"
        )
...
def test_converts_classic_xmind_outline() -> None:
    ...
    assert (
        _convert(_xmind(**{"content.xml": content}))
        == "# Classic Sheet\n\n- Root Topic\n  - Child\n    > Remember this"
    )
...
def test_omits_empty_topics_without_losing_children() -> None:
    content = [
        {
            "title": "Outline",
            "rootTopic": {"children": {"attached": [{"title": "Child"}]}},
        }
    ]

    assert (
        _convert(_xmind(**{"content.json": json.dumps(content).encode()}))
        == "# Outline\n\n- Child"
    )
...
def test_prefers_modern_content_when_both_formats_are_present() -> None:
    ...
    assert (
        _convert(
            _xmind(
                **{"content.json": json.dumps(modern).encode(), "content.xml": b"<bad"}
            )
        )
        == "# Modern\n\n- JSON"
    )
...
@pytest.mark.parametrize(
    "entries", [{}, {"content.json": b""}, {"content.json": b"not json"}]
)
